...
import { useGetVaultsQuery } from '@services/api';
...
const VaultSetUp = () => {
  ...
  const { data, error, isError, isLoading } = useGetVaultsQuery(0);
  console.log('Vaualtss vaulrs', data, error, isError);

  return (
    <>
      {isLoading && <LoadingSkeleton />}
      {!isLoading && isError && <div>Error Ocurred </div>}
      {!isLoading && !data?.data?.length && !isError && (
        <div>
          {createVault ? (
            <>
            ...
            <CreateVault setCreateVault={setCreateVault} />
          )}
        </div>
      )}
      {!isError && data?.data.length && (
        <div>
          <VaultList vaults={data?.data ?? []} />
        </div>
      )}
    </>
  );
...
const CreateVault = ({ setCreateVault }) => {
  return (
        ...
        <div className="pt-6 lg:pt-16 flex flex-col lg:flex-row justify-between gap-4 w-full">
          ...
          <div className="w-full lg:max-w-[50%] mx-auto">
            ...
            <button
              ...
              className="hidden lg:block font-paralucentMedium rounded-lg mt-10 text-sm leading-[14px] w-full py-4 bg-[#01A0FF] text-safekeep-white"
            >
              Create New Vault
            </button>
          </div>
...
const VaultList = ({ vaults }) => {
  ...
  return (
    <div className="container mx-auto">
      <h1 className="text-2xl  mb-4 text-[#333D8A]">Vault List</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {vaults.map((vault, index) => (
          <div key={index} className="bg-white p-4 shadow rounded-lg text-[#475467] text- ">
            <h2 className=" font-bold mb-2">{vault.vaultName}</h2>
            <p className=" mb-2">Backup Address: {truncateWalletAddress(vault.backupAddress)}</p>
            <p className="">
              Starting Balance: {vault.totalWeiAllocated} {nativeToken}
            </p>
          </div>
        ))}
      </div>
...
const LoadingSkeleton = () => {
  return (
    <div className="w-[70vw] max-w-3xl justify-center ">
      <h1 className=" mb-4 animate-pulse bg-loader-one rounded-full w-[6rem] h-8"></h1>
      <div className=" mt-16 lg:grid grid-cols-3  gap-3">
        {new Array(3).fill(0).map((vault, index) => (
          <div className="flex mb-3 col-span-1 bg-white p-4 shadow rounded-lg">
            <div className="flex-grow">
              <h2 className="bg-loader-one mb-2 w-3/4 h-5  rounded"></h2>
              <p className=" bg-loader-one mb-2 w-3/4 h-5  rounded "></p>
              <p className=" bg-loader-one mb-2 h-5  w-1/2 rounded"></p>
            </div>
          </div>
        ))}
      </div>
