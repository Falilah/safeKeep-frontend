...
const SingleInheritors = ({ ...props }) => {
  const { sigName, address, ethShare } = props;
  //   const balanceInEth = ethers.utils.formatEther(ethShare ?? 0);

  return (
    <div className="mx-auto flex flex-col gap-2.5 items-center w-full p-2.5 bg-[#FFFFFF] rounded-[10px] shadow-sm">
        ...
        <div className="flex gap-2 items-center">
          ...
          <div className="gap-1 flex flex-col">
            <div className="px-2 py-1 font-dmSans font-semibold text-[#0150B4] text-base bg-[#E6F1FF] rounded-3xl">
              eth: {truncateWalletAddress(address)}
            </div>
            <p className="font-dmSans text-[#B5B5B5] text-xs">{sigName}</p>
          </div>
        </div>
      ...
      <div className="w-full bg-[#F3FDFF] p-2.5 rounded-lg flex justify-between">
        <div>
          ...
          <div className="flex flex-col gap-1 mt-2">
            <p className="font-paralucentMedium text-xl text-[#001268]">{ethShare} ETH</p>
          </div>
        </div>
